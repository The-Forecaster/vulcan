...
fun Wrapper.clientSend(text: String, pre: Boolean = true) {
    minecraft.inGameHud.chatHud.addMessage(Text.of(if (pre)"$prefix $text" else text))
}

fun Wrapper.error(text: String, pre: Boolean = true) {
    minecraft.inGameHud.chatHud.addMessage(Text.of(if (pre) "$prefix${modifier}c $text" else "${modifier}c$text"))
}
